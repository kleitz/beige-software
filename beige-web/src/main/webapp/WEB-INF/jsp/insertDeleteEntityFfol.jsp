...
<c:set var="actTitle" value="Delete"/>
<c:set var="nameAction" value="entityFfolDelete" scope="request"/>
<c:set var="wdgName" value="wdgDeleteForm" scope="request"/>
<c:set var="isCreate" value="false"/>
<c:set var="msgSuccess" value="deleted_ok" scope="request"/>
<c:if test="${entity.isNew}">
  <c:set var="actTitle" value="Create"/>
  <c:set var="wdgName" value="wdgNewForm" scope="request"/>
  <c:set var="nameAction" value="entityFfolSave" scope="request"/>
...
<dialog id="${namePlaceSubForm}EditDlg" class="dlg dlg-sub" oncancel="return false;">
  <div class="form-std">
    ...
    <form id="${namePlaceSubForm}EditFrm" action="uploadSingle/" method="POST" enctype="multipart/form-data">
      <input type="hidden" name="nmsAct" value="${nameAction},entityEdit,list">
      ...
      <div class="form-actions">
        ...
        <button type="button" onclick="submitFormByAjax('${namePlaceSubForm}EditFrm', ${isCreate});">${srvI18n.getMsg(actTitle)}</button>
