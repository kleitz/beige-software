...
<div class="input-line">
  ...
  <c:if test="${parVal eq 'PAYMENTTOTAL/docName.ITSTOTAL gte 0.30'}"><c:set var="selectedGteq30" value="selected"/></c:if>
  <c:if test="${parVal eq 'PAYMENTTOTAL/docName.ITSTOTAL gte 0.50'}"><c:set var="selectedGteq50" value="selected"/></c:if>
  <c:if test="${parVal eq 'PAYMENTTOTAL/docName.ITSTOTAL gte 1'}"><c:set var="selectedGteq100" value="selected"/></c:if>
  ...
  <div class="input-line">
    <select name="${parVal}" onchange="filterOperChanged(this, '${parVal}');">
      ...
      <option value="PAYMENTTOTAL/${docName}.ITSTOTAL gte 0.30" ${selectedGteq5}>&gt;= 30%</option>
      <option value="PAYMENTTOTAL/${docName}.ITSTOTAL gte 0.50" ${selectedGteq5}>&gt;= 50%</option>
      <option value="PAYMENTTOTAL/${docName}.ITSTOTAL gte 1" ${selectedGteq5}>&gt;= 100%</option>
